...
;                     [transmit-string a-chan "1tkr0115" ]
;                     
;                     
;                     [yield]
;                     ;[send ssh send-string "1tkr0115"]
;                     [transmit-string a-chan "1tkr0115" ]
;                     
;                     
...
[define ssh-wrapper% 
  (class object%
    ...
    (field [sess #f] [chan #f] [server-address #f] [user #f] [password #f] [receiver-thread #f] [transmitter-thread #f] [transcript ""] [echo-to-stdout #t] [timeout 60][procvals #f][writeport #f][readport #f][killfunc #f]) ; field
...
[define ssh-command [lambda [host user pass command]
                      [ssh-script "aaa" "aaa" host user pass [thunk 
                                                         [sn command] 
                                                         [wsn "assword:" pass] 
                        [sleep 15]
